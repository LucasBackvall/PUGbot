...
client.once("ready", () => {
  try {
    ...
    last = "Unable to load last PUG. This is because the data-file couldn't be read";
    link = "Unable to load invite link. This is because the data-file couldn't be read";
    ch = ".";
  }
...
client.on("message", message => {
  try{
    // Disallow commands outside of specified channel. DM commands are also disallowed so you can't enter a list without people seeing it.
    // TODO: Allow some commands in DM. If a report system is implemented you should be able to report players in DM to bot.
    if (message.channel.type != "text" || ch != "." && ch != message.channel.name) return;
    ...
    switch (command) {
      ...
      case "p":
        if (args.length == 0) message.channel.send("@here\n" + listactive())
          ...
          var msg = "@here\n```fix\nFollowing PUG(s) were promoted by "+message.author.username+"\n";
          args.forEach(function (arg) {
            for (var i = 0; i < pugs.length; i++) {
